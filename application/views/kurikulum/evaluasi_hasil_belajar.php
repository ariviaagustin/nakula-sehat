        ...
        <div class="row">
          <div class="col-md-12">
            <?php if($get_catatan){ ?>
              ...
              <hr>
            <?php } ?>
            <h5><b>d. Evaluasi Hasil Belajar<span style="color: red;">*</span></b></h5><br>
            <form action="<?= site_url('Kurikulum/aksi_isi_subbab_kurikulum'); ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
              <div class="row form-group">
                <div class="col-12 col-md-12">
                  <input type="text" class="form-control" name="evaluasi_hasil_belajar" required="" value="<?= $kurikulum->evaluasi_hasil_belajar; ?>">
                </div>
              </div>
              <?php $total = count($evaluasi_hasil_belajar); ?>
              <?php if($total == 0){ ?>
                <div class="col-md-12 isian_multiple">
                  <div class="form-group multiple-form-group-upload">
                    <div class="form-group m-form__group">
                      <div class="row form-group">
                        <div class="col-12 col-md-12">
                          <input type="text" class="form-control" name="isi_evaluasi_hasil_belajar[]" required placeholder = "(Evaluasi Hasil Belajar)">
              ...
              <?php } ?>
              <?php if($total >= 1){ $ds = 1; foreach ($evaluasi_hasil_belajar as $val) { ?>
                <input type="hidden" name="id_isi_evaluasi_hasil_belajar[]" value="<?= $val->id_isi_evaluasi_hasil_belajar; ?>">
                <div class="col-md-12" id="id_upload">
                  <div class="form-group multiple-form-group-upload">
                    <div class="form-group m-form__group">
                      <div class="row form-group">
                        <div class="col-12 col-md-12">
                          <input type="text" class="form-control" name="isi_evaluasi_hasil_belajar[]" value="<?php echo $val->isi_evaluasi_hasil_belajar; ?>" required>
                          <span style="color: blue;">Harap diisi satu persatu. Apabila isian lebih dari satu, dapat menambahkan kolom dengan klik tombol tambah disebelah kanan</span>
                        </div>
                      </div>
                      <span class="input-group-btn">
                        <button type="button"  class="btn btn-<?= ($ds-1 < $total-1) ? 'danger btn-remove hapus-multiple' : 'success btn-add'?>" data-title="d" data-awal="1" data-id="<?=$val->id_isi_evaluasi_hasil_belajar?>" style="float: right;"><?= ($ds-1 < $total-1) ? '-' : '+'?></button>
                      </span>    
                      <div class="clearfix"></div>           
                    </div>
                  </div>
